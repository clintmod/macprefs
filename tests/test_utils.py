...
def test_copy_dir(execute_shell_mock, level_mock):
    ...
    execute_shell_mock.assert_called_with(["rsync", "-a", "-vv", "src", "dest"])


@patch("utils.execute_shell")
def test_copy_dir_works_with_sudo(execute_shell_mock):
    utils.copy_dir("src", "dest", with_sudo=True)
    execute_shell_mock.assert_called_with(["sudo", "rsync", "-a", "src", "dest"])
...
def test_copy_files(execute_shell_mock, log_mock):
    ...
    execute_shell_mock.assert_called_with(["rsync", "-a", "-vv"] + files + [dest])
...
def test_change_owner(execute_shell_mock):
    ...
    execute_shell_mock.assert_called_with(["sudo", "chown", "-R", "clint", ssh_dir], suppress_errors=True)
...
def test_change_mode(execute_shell_mock):
    ...
    execute_shell_mock.assert_called_with(["sudo", "chmod", "-R", "600", ssh_dir], suppress_errors=True)
...
def test_ensure_subdirs_listable(execute_shell_mock):
    ...
    execute_shell_mock.assert_called_with(["sudo", "chmod", "-R", "a+X", ssh_dir], suppress_errors=True)
...
def test_change_owner_for_files(shell_mock):
    ...
    shell_mock.assert_called_with(["sudo", "chown", user] + files, suppress_errors=True)
...
def test_change_mode_for_files(shell_mock):
    ...
    shell_mock.assert_called_with(["sudo", "chmod", mode] + files, suppress_errors=True)
...
def test_run_permission_change_tolerates_failure(shell_mock, warning_mock):
    ...
    shell_mock.assert_called_with(["sudo", "chown", "clint", "/protected"], suppress_errors=True)
...
def test_restart_cfprefsd(execute_shell_mock):
    ...
    execute_shell_mock.assert_called_with(["sudo", "killall", "cfprefsd"], suppress_errors=True)
...
def test_copy_file(shell_mock, level_mock):
    ...
    shell_mock.assert_called_with(["rsync", "-a", "-vv", fle, dest])
