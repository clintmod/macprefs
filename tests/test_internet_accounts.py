...
def test_restore_internet_accounts(copy_mock, owned_mock):
    ...
    copy_mock.assert_called_with(config.get_internet_accounts_backup_dir(), config.get_internet_accounts_dir(), with_sudo=True)
    owned_mock.assert_called_with(config.get_internet_accounts_dir(), config.get_user())
